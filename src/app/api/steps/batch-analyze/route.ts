...
const OPENROUTER_MODEL = process.env.OPENROUTER_MODEL || 'z-ai/glm-5.3-flash';
...
async function callOpenRouterForBatch(prompt: string, data: string, mime: string): Promise<string> {
  try {
    ...
    const msg = String(e);
    if ((msg.includes('404') || msg.includes('429') || msg.includes('500') || msg.includes('502') || msg.includes('503')) && OPENROUTER_MODEL_2 && OPENROUTER_MODEL_2 !== OPENROUTER_MODEL) {
...
async function analyzeOneImage(imageBase64: string, expectedDate: string, hintInput: string = 'auto', explicitModel: string = '') {
  ...
  const prompt = `คุณคือผู้ช่วยตรวจสอบภาพสำหรับโครงการส่งเสริมสุขภาพ "นับก้าวเดิน" วิเคราะห์ภาพแคปหน้าจอแอปนับก้าว (step counter) แล้วตอบเป็น JSON เท่านั้น
...
4. ให้คะแนนความมั่นใจ 0.0-1.0 ว่าจำนวนก้าวที่อ่านได้แม่นยำ — 1.0=มั่นใจสูงมาก ตัวเลข+วันที่ชัดเจนตรงกัน ไม่มีร่องรอยตัดต่อ, <0.8=มีข้อสงสัยเล็กน้อย
  ...
  try {
    if (hint === 'openrouter') {
      ...
      try {
        ...
        const msg = String(e);
        if ((msg.includes('404') || msg.includes('429') || msg.includes('500') || msg.includes('502') || msg.includes('503')) && OPENROUTER_MODEL_2 && m !== OPENROUTER_MODEL_2) {
      ...
      try {
        ...
        const msg = String(e);
        if ((msg.includes('404') || msg.includes('429') || msg.includes('500') || msg.includes('502') || msg.includes('503')) && m !== OPENROUTER_MODEL) {
      ...
      try {
        ...
        if ((status === 404 || status === 429 || status === 503 || status === 500) && OPENROUTER_API_KEY) {
