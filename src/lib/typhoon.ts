...
function buildPrompt(ctx: TyphoonPromptContext): { system: string; user: string } {
  ...
  const user =
    ...
    `3. step_count: อ่านเฉพาะตัวเลขก้าวรวมหลักที่อยู่ใกล้คำว่า "ก้าวเดิน" เท่านั้น (เช่น ก้าวเดิน 5,546 → 5546) ถอด comma ออก ถ้าไม่เห็นให้ null\n` +
    `   ห้ามอ่าน "จำนวนชั้นที่ขึ้น 1" หรือ "58/240 กิโลแคล" หรือ "2.19 กม." — เลข 1 ตัวเล็กใต้คำว่า "จำนวนชั้นที่ขึ้น" ไม่ใช่ก้าว ให้มองหาเลขใหญ่ 4 หลักที่อยู่ใต้คำว่า "ก้าวเดิน" ตรงกลางจอเท่านั้น\n` +
    `   ตัวอย่าง: ในภาพมี "ก้าวเดิน\\n5,546" และ "จำนวนชั้นที่ขึ้น\\n1" ให้ตอบ 5546 ไม่ใช่ 1\n` +
