...
function buildPrompt(ctx: TyphoonPromptContext): { system: string; user: string } {
  ...
  const user =
    ...
    `1. raw_date_text_from_image: คัดลอก "ข้อความวันที่" ที่ตาเห็นจริงๆ ในภาพเท่านั้น ห้ามเอาข้อความอื่นทั้งหน้า\n` +
    `   ตัวอย่างที่ถูกต้อง: "27 ส.ค. 2569" หรือ "พ. 2 ก.ย." หรือ "Today" หรือ "วันนี้" — ไม่ใช่ "วันพฤหัสบดีที่ 27 ส.ค. 2569 เคลื่อนไหว 98/240..."\n` +
    ...
    `2. parsed_date_from_image: แปลง raw_date_text_from_image เป็น YYYY-MM-DD (พ.ศ.→ค.ศ. -543, ไม่มีปี → ใช้ปี ${ctx.currentYear})\n` +
    `3. step_count: อ่านเฉพาะตัวเลขก้าวรวมหลักที่อยู่ใกล้คำว่า ก้าว/ก้าวเดิน/steps (เช่น ก้าวเดิน 4,579 หรือ 4,579 ก้าว → 4579) ถอด comma ออก ถ้าไม่เห็นให้ null\n` +
    `   อย่าสับสนกับ kcal, km, นาที, ชั้น\n` +
    `\n[ตัวอย่างผลลัพธ์ที่ถูกต้อง]\n` +
    `ภาพมี "ก้าวเดิน 4,579" และ "27 ส.ค. 2569" → {"raw_date_text_from_image":"27 ส.ค. 2569","parsed_date_from_image":"2026-08-27","step_count":4579,"ocr_confidence":0.92,"visual_evidence":"พบ '27 ส.ค. 2569' มุมบน, พบ 'ก้าวเดิน 4,579' กลางจอ"}\n` +
