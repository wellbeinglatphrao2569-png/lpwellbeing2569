...
function buildPrompt(ctx: TyphoonPromptContext): { system: string; user: string } {
  ...
  const user =
    ...
    `3. step_count: อ่านเฉพาะ "จำนวนก้าว" ตัวใหญ่ตรงกลางการ์ดสีขาวเท่านั้น (ใต้คำว่า "จำนวนก้าว" จะมีเลขใหญ่ เช่น 3,231 และบรรทัดล่าง "/6,000" คือเป้าหมาย ห้ามอ่าน /6,000)\n` +
    `   ตัวอย่าง: การ์ดแสดง "จำนวนก้าว\\n3,231\\n/6,000" ให้ตอบ 3231 ไม่ใช่ 6000\n` +
    `   ห้ามอ่าน "จำนวนชั้นที่ขึ้น 1" หรือ "34 นาที" หรือ "101 แคล" หรือตัวเลขกิจกรรมย่อยข้างล่าง ("237 ก้าว" / "483 ก้าว" / "1,052 ก้าว") — ให้อ่านยอดรวมบนสุดเท่านั้น\n` +
